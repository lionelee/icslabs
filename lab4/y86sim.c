...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
	...
	if (val < 0) sign = TRUE;
    if(op == A_ADD){
		if (((argA < 0) == (argB < 0)) && ((val < 0) != (argB < 0))) ovf=TRUE;
    }
	else if(op == A_SUB){
		if (((argA > 0) == (argB < 0)) && ((val < 0) != (argB < 0))) ovf=TRUE;
	}
...
stat_t nexti(y86sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_RRMOVL:  /* 2:x regA:regB */
		...
		if((cond_t)ifun < C_YES || (cond_t)ifun > C_G) return STAT_INS;
      ...
      case I_JMP: /* 7:x imm */
		if((cond_t)ifun < C_YES || (cond_t)ifun > C_G) return STAT_INS;
		sim->pc = (cond_doit(sim->cc, (cond_t)ifun))? imm : next_pc;
		break;
	
      case I_CALL: /* 8:x imm */
		if((cond_t)ifun < C_YES || (cond_t)ifun > C_G) return STAT_INS;
		R_esp = get_reg_val(sim->r, REG_ESP);
		R_esp -= 4;
		set_reg_val(sim->r, REG_ESP, R_esp);
		if(!set_long_val(sim->m, R_esp, next_pc))
		{
			err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, R_esp);
			return STAT_ADR;
		}
		sim->pc = imm;
		break;
      ...
      case I_RET: /* 9:0 */
		...
		R_esp = get_reg_val(sim->r, REG_ESP);
		if(!get_long_val(sim->m, R_esp, &sim->pc))
		{
			err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, R_esp);
			return STAT_ADR;
		}
      ...
      case I_PUSHL: /* A:0 regA:F */
		...
		R_esp -= 4;
		set_reg_val(sim->r, REG_ESP, R_esp);
		if(!set_long_val(sim->m, R_esp, val))
		{
			err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, R_esp);
			return STAT_ADR;
		}
      ...
      case I_POPL: /* B:0 regA:F */
        if(ifun != 0 || !NORM_REG(regA) || !NONE_REG(regB)) return STAT_INS;
		R_esp = get_reg_val(sim->r, REG_ESP);
		if(!get_long_val(sim->m, R_esp, &val))
		{
			err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, R_esp);
			return STAT_ADR;
		}
