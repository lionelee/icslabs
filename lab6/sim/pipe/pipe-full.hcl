...
#########################################                                      
#	STAGE	#       iaddl V, rB   		#	# add iaddl into valid instruction		
#########################################
#			#	icode:ifun <- M1[PC]	#	
#	Fetch	#	rA:rB <- M1[PC+1]		#	# iaddl need register
# 			#	valC <- M4[PC+2]		#	# iaddl need valC
#			#	valP <- PC+6    		#
#########################################
#  Decode	#	valB <- R[rB]			#	# d_srcB = rB
#########################################
#  Execute	#	valE <- valB + valC 	#	# aluA = E_valC, aulB = E_valB and set CC
#########################################
#  Memory	#							#	
#########################################
# Write Back# 	R[rB] <- valE			#	# dstE = rB
#########################################
# PC Update #	PC <- valP				#	
#########################################
  
#########################################                                      
#	STAGE	#          leave 	  		#	# add leave into valid instruction
#########################################
#	Fetch	#	icode:ifun <- M1[PC]	#
#			#	valP <- PC+1    		#
#########################################
#  Decode	#	valA <- R[%ebp]			#	# d_srcA = REBP
#			#	valB <- R[%ebp]			#	# d_srcB = REBP
#########################################
#  Execute	#	valE <- valB + 4	 	#	# aluA = 4, aluB = E_valB
#########################################
#  Memory	#	valM <- M4[valA]		#	# mem_addr = M_valA, mem_read = TRUE
#########################################
# Write Back# 	R[%esp] <- valE			#	# dstE = RESP
#			# 	R[%ebp]	<- valM			#	# dstM = REBP
#########################################
# PC Update #	PC <- valP				#
#########################################

# modify hcl as follows to realize forwarding between mrmovl and  rmmovl, pushl
# modify e_valA as [ E_icode in { IRMMOVL, IPUSHL }  && E_srcA == M_dstM : m_valM;
#					1 : E_valA; ]
# using E_icode in { IMRMOVL, IPOPL } && E_dstM in { d_srcA, d_srcB } && 
# !D_icode in { IRMMOVL, IPUSHL }
# to modify F_stall, D_stall, D_bubble, E_bubble 
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) ||
	#(E_icode == IJXX && !e_Cnd && !M_icode==IRMMOVL && !W_icode==IOPL) ||
	# Stalling at fetch while ret passes through pipeline
...
bool E_bubble =
	...
	# Conditions for a load/use hazard
	E_icode in { IMRMOVL, IPOPL } &&
	 E_dstM in { d_srcA, d_srcB} && !D_icode in { IRMMOVL, IPUSHL };
